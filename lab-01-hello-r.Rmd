...
This is a data frame with 1846 rows and 3 variables. The variable names are **dataset**, **x**, and **y**
...
The correlation coefficient is technically nonexistent because the plot isn't linear, but the answer would be -0.06447185	
...
The correlation for the star dataset is -0.0629611	which is very similar to that of the dino dataset.
...
```{marginfigure}
Start with `star_data` and calculate a summary statistic that we will call `r` as the `cor`relation between `x` and `y`.
```
...
The value of the correlation coefficient is -0.06834336	which again is very close to that of dino and star's dataset. This is likely because none of them have been set linear paths.
...
```
```{marginfigure}
Start with `circle_data` and calculate a summary statistic that we will call `r` as the `cor`relation between `x` and `y`.
```
...
Could not complete either of the last steps. There is no option to select "Output Options," There is only expand, collapse, clear, and clear all output.
